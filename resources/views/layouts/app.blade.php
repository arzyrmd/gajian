...
<body class="bg-slate-50 text-slate-900 font-sans min-h-screen flex flex-col antialiased">
    ...
    <nav class="no-print bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex flex-1">
                    <div class="flex-shrink-0 flex items-center">
                        <a href="{{ auth()->check() && auth()->user()->is_admin ? route('monitoring.index') : route('harian') }}" class="flex items-center space-x-2">
                        ...
                        @if(Auth::user()->is_admin)
                            ...
                            <a href="{{ route('monitoring.index') }}" 
                               class="px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('monitoring.*') ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900' }}">
                                <span class="flex items-center space-x-2">
                                    <svg class="w-4.5 h-4.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5L1.5 9.75M3.75 3h16.5M20.25 3v11.25A2.25 2.25 0 0118 16.5h-2.25M20.25 3h1.5L22.5 9.75M12 18.75h.008v.008H12v-.008zM12 21h.008v.008H12V21zM9.75 21h.008v.008H9.75V21zm4.5 0h.008v.008h-.008V21z"></path></svg>
                            ...
                            <a href="{{ route('tarif.index') }}" 
                               class="px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('tarif.index') ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900' }}">
                                <span class="flex items-center space-x-2">
                                    <svg class="w-4.5 h-4.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.43l-1.003.828c-.293.241-.438.613-.43.992a7.723 7.723 0 010 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 01-.22.128c-.335.133-.75.072-1.076-.124l-1.217.456a1.125 1.125 0 01-1.37-.49l-1.296-2.247a1.125 1.125 0 01.26-1.43l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 010-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28z"></path><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                                    <span>Tarif</span>
                                </span>
                            </a>
                            <a href="{{ route('users.index') }}" 
                               class="px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 {{ request()->routeIs('users.index') ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900' }}">
                                <span class="flex items-center space-x-2">
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div class="relative">
                        ...
                        <div id="profile-dropdown-menu" 
                             class="opacity-0 scale-95 pointer-events-none transform origin-top-right absolute right-0 mt-2 w-56 rounded-2xl border border-slate-100 bg-white shadow-xl shadow-slate-200/50 py-1.5 z-50 transition-all duration-200 ease-out">
                            <!-- User Header -->
        ...
        @auth
        <div id="mobile-menu" 
             style="max-height: 0px; opacity: 0; padding-top: 0px; padding-bottom: 0px;"
             class="sm:hidden border-t border-slate-200 bg-white transition-all duration-350 ease-in-out overflow-hidden px-4">
            <div class="space-y-1.5 pb-2">
                @if(Auth::user()->is_admin)
                    ...
                    <a href="{{ route('users.index') }}" 
                       class="flex items-center space-x-3 px-4 py-3 rounded-xl text-base font-medium {{ request()->routeIs('users.index') ? 'bg-blue-50 text-blue-600 shadow-sm' : 'text-slate-600 hover:bg-slate-50' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"></path></svg>
                        <span>Manage User</span>
                    </a>
                @else
    ...
    <!-- Bottom Tab Navigation (Mobile Only) Sticky -->
    @auth
    <div class="sm:hidden no-print fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t border-slate-200 py-2.5 z-40 px-4 flex justify-around items-center shadow-[0_-5px_15px_rgba(0,0,0,0.05)]">
        @if(Auth::user()->is_admin)
            <!-- Admin Bottom Links -->
            <a href="{{ route('monitoring.index') }}" 
               class="flex flex-col items-center space-y-0.5 w-16 {{ request()->routeIs('monitoring.*') ? 'text-blue-600 font-semibold' : 'text-slate-450' }}">
                <svg class="w-5.5 h-5.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 3v11.25A2.25 2.25 0 006 16.5h2.25M3.75 3h-1.5L1.5 9.75M3.75 3h16.5M20.25 3v11.25A2.25 2.25 0 0118 16.5h-2.25M20.25 3h1.5L22.5 9.75M12 18.75h.008v.008H12v-.008zM12 21h.008v.008H12V21zM9.75 21h.008v.008H9.75V21zm4.5 0h.008v.008h-.008V21z"></path></svg>
                <span class="text-[9px]">Monitor</span>
            </a>
            <a href="{{ route('tarif.index') }}" 
               class="flex flex-col items-center space-y-0.5 w-16 {{ request()->routeIs('tarif.index') ? 'text-blue-600 font-semibold' : 'text-slate-450' }}">
                <svg class="w-5.5 h-5.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.324.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 011.37.49l1.296 2.247a1.125 1.125 0 01-.26 1.43l-1.003.828c-.293.241-.438.613-.43.992a7.723 7.723 0 010 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.954.26 1.43l-1.298 2.247a1.125 1.125 0 01-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 01-.22.128c-.335.133-.75.072-1.076-.124l-1.217.456a1.125 1.125 0 01-1.37-.49l-1.296-2.247a1.125 1.125 0 01.26-1.43l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 010-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 01-.26-1.43l1.297-2.247a1.125 1.125 0 011.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28z"></path><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>
                <span class="text-[9px]">Tarif</span>
            </a>
            <a href="{{ route('users.index') }}" 
               class="flex flex-col items-center space-y-0.5 w-16 {{ request()->routeIs('users.index') ? 'text-blue-600 font-semibold' : 'text-slate-450' }}">
                <svg class="w-5.5 h-5.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"></path></svg>
                <span class="text-[9px]">User</span>
            </a>
        @else
            <!-- Technician Bottom Links -->
            <a href="{{ route('harian') }}" 
               class="flex flex-col items-center space-y-0.5 w-16 {{ request()->routeIs('harian') ? 'text-blue-600 font-semibold' : 'text-slate-450' }}">
                <svg class="w-5.5 h-5.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5m-9-6h.008v.008H12v-.008zM12 15h.008v.008H12V15zm0 2.25h.008v.008H12v-.008zM9.75 15h.008v.008H9.75V15zm0 2.25h.008v.008H9.75v-.008zM7.5 15h.008v.008H7.5V15zm0 2.25h.008v.008H7.5v-.008zm6.75-4.5h.008v.008h-.008v-.008zm0 2.25h.008v.008h-.008V15zm0 2.25h.008v.008h-.008v-.008zm2.25-4.5h.008v.008H16.5v-.008zm0 2.25h.008v.008H16.5V15z"></path></svg>
                <span class="text-[9px]">Harian</span>
            </a>
            <a href="{{ route('bulanan') }}" 
               class="flex flex-col items-center space-y-0.5 w-16 {{ request()->routeIs('bulanan') ? 'text-blue-600 font-semibold' : 'text-slate-450' }}">
                <svg class="w-5.5 h-5.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"></path></svg>
                <span class="text-[9px]">Bulanan</span>
            </a>
        @endif
    </div>
    @endauth
    ...
    <!-- Main Content Area -->
    <main class="flex-1 py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full pb-20 sm:pb-8">
