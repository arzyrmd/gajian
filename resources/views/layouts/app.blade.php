...
<body class="bg-slate-50 text-slate-900 font-sans min-h-screen flex flex-col antialiased">
    ...
    <nav class="no-print bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
            ...
            <div class="flex justify-between h-16">
                ...
                <div class="flex flex-1">
                    <div class="flex-shrink-0 flex items-center">
                        <a href="{{ route('harian') }}" class="flex items-center space-x-2">
                ...
                <div class="flex items-center space-x-4">
                    <div class="hidden md:flex flex-col text-right">
                        ...
                        <span class="text-xs text-slate-500 leading-none mt-1">
                            @if(Auth::user()->is_admin)
                                <span class="bg-indigo-50 text-indigo-700 font-medium px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider">Admin</span>
                            @else
                                <span class="bg-blue-50 text-blue-700 font-medium px-1.5 py-0.5 rounded text-[10px] uppercase tracking-wider">Teknisi</span>
                            @endif
                        </span>
                    </div>

                    <!-- Profile Circle -->
                    <div class="w-10 h-10 rounded-full bg-slate-200 border border-slate-300 flex items-center justify-center text-slate-700 font-bold">
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" 
                                class="p-2 rounded-xl text-slate-500 hover:bg-rose-50 hover:text-rose-600 transition-all duration-200 cursor-pointer" 
                                title="Keluar">
                            <svg class="w-5.5 h-5.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0013.5 3h-6a2.25 2.25 0 00-2.25 2.25v13.5A2.25 2.25 0 007.5 21h6a2.25 2.25 0 002.25-2.25V15M12 9l-3 3m0 0l3 3m-3-3h12.75"></path></svg>
                        </button>
                    </form>
        ...
        @auth
        <div id="mobile-menu" class="hidden sm:hidden border-t border-slate-200 bg-white px-4 py-3 space-y-2">
            <a href="{{ route('harian') }}" 
               class="block px-3 py-2.5 rounded-xl text-base font-medium {{ request()->routeIs('harian') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50' }}">
                Harian
            </a>
            <a href="{{ route('bulanan') }}" 
               class="block px-3 py-2.5 rounded-xl text-base font-medium {{ request()->routeIs('bulanan') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50' }}">
                Bulanan
            </a>
            @if(Auth::user()->is_admin)
            <a href="{{ route('tarif.index') }}" 
               class="block px-3 py-2.5 rounded-xl text-base font-medium {{ request()->routeIs('tarif.index') ? 'bg-blue-50 text-blue-600' : 'text-slate-600 hover:bg-slate-50' }}">
                Pengaturan Tarif
            </a>
            @endif
            <div class="border-t border-slate-150 pt-3 flex items-center justify-between">
                ...
                <span class="text-xs px-2 py-1 rounded bg-slate-100 font-semibold text-slate-600">
